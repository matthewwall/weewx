...
# These dictionaries control how different observation types are handled. They
# can be manipulated by exctensions. They only list the exceptions to the defaults.

# The type of accumulator to use. The default is ScalarStats. 
# No exceptions in weecore
init_dict = {}

# What to do when adding a record. The default is Accum.add_value.
# Two exceptions:
add_record_dict = {'usUnits'   : Accum.check_units,
                   'dateTime'  : Accum.noop}

# How to extract a value from the accumulator. Default is Accum.avg_extract.
# No exceptions in weecore.
extract_dict = {}
